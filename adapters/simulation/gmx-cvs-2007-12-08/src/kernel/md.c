/*
 * $Id: md.c,v 1.1 2008-06-27 10:50:09 mdd Exp $
 * 
	     ...
	     unsigned long Flags)
{
  ...
  while ((!bRerunMD && (step_rel <= ir->nsteps)) ||  
    ...
    // MDDriver (July 2007) - insert 6 lines
    bX = (do_per_step(step,nstximd) || bLastStep);
    if ( bX ) {
      gimd_send_traj( cr, mdatoms, 
		      step, t,
	              mdatoms->nr, state->x, state->v, f, state->box );
   }
    ...
    */
    ...
    // MDDriver (July 2007) - insert 
    bool send_ener = (do_per_step(step,nstximd) || bLastStep);
    if ( send_ener ) {
      gimd_send_energies( cr, step, mdebin->ebin, mdebin->ie, 
			  mdebin->iconrmsd - mdebin->ie );
    }
    if( gimd_stop_simulation() != 0 ) {
      // Stop Gromacs
      ir->nsteps = step_rel - 1;
    }
    // MDDriver (July 2007) - End insertion 
  ...
  // MDDriver (July 2007) - insert 1 line
  gimd_terminate( cr );

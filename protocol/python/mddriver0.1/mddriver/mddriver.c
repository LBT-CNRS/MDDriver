...
PyDoc_STRVAR(mddriver__doc__,"Python interface to MDDriver");

PyDoc_STRVAR(mddriver_client__doc__,"Must be call before by the client before starting a interactive molecular dynamics. ");

PyDoc_STRVAR(mddriver_server__doc__,"Must be call before by the server (simulation code) and by the client before starting a interactive molecular dynamics. ");
...
initmddriver(void)
{
	...
	Py_InitModule3("mddriver", mddriver_methods,mddriver__doc__);
}
